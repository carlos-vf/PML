...
def run_pipeline(config):
    ...
    seeds = config.get('seeds', [27, 272, 2727, 1, 30])
    noise_scale = config.get('noise_scale', 0.0)
    label_noise_scale = config.get('label_noise_scale', 0.0)
    label_noise_range = config.get('label_noise_range', (0.0, 0.5))
    ...
    if df_cfg is not None:
        ...
        for n_estimators_df in df_n_estimators_list:
            for n_trees_drf in df_n_trees_list:
                accuracies_DF = []
                for seed in seeds:
                    set_all_seeds(seed)
                ...
                records.append({
                    ...
                    'n_trees_drf': n_trees_drf,
                    'mean_accuracy': mean_acc,
                    'std_accuracy': std_acc
                })
                print(f"DeepForest n_estimators={n_estimators_df}, n_trees={n_trees_drf}: {mean_acc:.4f} ± {std_acc:.4f}")
        ...
        df_df['combo_label'] = df_df.apply(
            lambda row: f"e{row['n_estimators']}_t{row['n_trees_drf']}", axis=1
        )
        ...
        best_configs.append({
            'Model': 'DeepForest',
            'hyperparams': f"n_estimators={int(best_row['n_estimators'])}, n_trees={int(best_row['n_trees_drf'])}",
            'mean_accuracy': best_row['mean_accuracy'],
            'std_accuracy': best_row['std_accuracy']
        })
    ...
    if pdrf_cfg is not None:
        ...
        records = []
        for n_cascade in pdrf_n_cascade_list:
            for n_trees_pdrf in pdrf_n_trees_list:
                for max_depth_pdrf in pdrf_max_depth_list:
                    # Validate
                    if max_depth_pdrf is not None and not isinstance(max_depth_pdrf, int):
                        raise ValueError(f"pdrf max_depth must be int, got {max_depth_pdrf!r}")
                    key = f"c{n_cascade}_t{n_trees_pdrf}_md{max_depth_pdrf}"
                    accuracies_PDRF = []
                    for seed in seeds:
                        set_all_seeds(seed)
                    ...
                    records.append({
                        ...
                        'n_trees_pdrf': n_trees_pdrf,
                        'max_depth_pdrf': max_depth_pdrf,
                        'mean_accuracy': mean_acc,
                        'std_accuracy': std_acc
                    })
                    print(f"PDRF {key}: {mean_acc:.4f} ± {std_acc:.4f}")

        df_pdrf = pd.DataFrame(records)
        csv_path = os.path.join(tables_dir,
                                f"{base_name}_{noise_filename_suffix}_pdrf_grid_({label_noise_range[0]},{label_noise_range[1]}).csv")
        df_pdrf.to_csv(csv_path, index=False)
        print(f"Saved PDRF grid CSV to {csv_path}")

        # Plot
        fig, ax = plt.subplots(figsize=(10,6))
        df_pdrf['combo_label'] = df_pdrf.apply(
            lambda row: f"c{row['n_cascade_estimators']}_t{row['n_trees_pdrf']}_md{row['max_depth_pdrf']}", axis=1
        )
        sns.barplot(data=df_pdrf, x='combo_label', y='mean_accuracy', ax=ax, errorbar=None)
        x_positions = range(len(df_pdrf))
        ax.errorbar(x_positions, df_pdrf['mean_accuracy'].values,
                    yerr=df_pdrf['std_accuracy'].values, fmt='none',
                    ecolor='black', elinewidth=1.5, capsize=5)
        ax.set_xlabel("PDRF config")
        ax.set_ylabel("Mean Accuracy")
        ax.set_ylim(0,1.05)
        plt.xticks(rotation=45, ha='right')
        sns.despine()
        for x, (m,s) in enumerate(zip(df_pdrf['mean_accuracy'], df_pdrf['std_accuracy'])):
            ax.text(x, m + s + 0.02, f'{m:.2f}±{s:.2f}', ha='center', va='bottom', fontsize=8)
        fig.suptitle(f"PDRF grid on {title_name}, noise {noise_scale} ({noise_type})", y=1.02)
        plt.tight_layout()
        plot_path = os.path.join(plots_dir,
                                 f"{base_name}_{noise_filename_suffix}_pdrf_grid_({label_noise_range[0]},{label_noise_range[1]}).png")
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close(fig)
        print(f"Saved PDRF grid plot to {plot_path}")

        # Determine best
        best_row = df_pdrf.loc[df_pdrf['mean_accuracy'].idxmax()]
        best_configs.append({
            'Model': 'PDRF',
            'hyperparams': f"n_cascade_estimators={int(best_row['n_cascade_estimators'])}, "
                           f"n_trees_pdrf={int(best_row['n_trees_pdrf'])}, max_depth_pdrf={best_row['max_depth_pdrf']}",
            'mean_accuracy': best_row['mean_accuracy'],
            'std_accuracy': best_row['std_accuracy']
        })
    ...
    if best_configs:
        ...
        sns.barplot(data=df_best, x='Model', y='mean_accuracy', ax=ax, errorbar=None,
                    palette=["#55bfc7" if m=='PDRF' else "lightgray" for m in df_best['Model']])
